...
export function BuyWithNear({
  ...
  const [ownedTokens, setOwnedTokens] = useState<object[]>([]);
  ...
  const singleBuy = async () => {
    ...
    if (tokenId) {
      ...
      for (let i = 0; i < 10; i++) {
        setTimeout(() => logResult(executionOutcome as FinalExecutionOutcome), i * 10000);
      }

      await fetchOwnedNfts(wallet.id, nftContractId || "");
        
      return executionOutcome as FinalExecutionOutcome;
    }
  };

  const logResult = (executionOutcome : FinalExecutionOutcome) => {
